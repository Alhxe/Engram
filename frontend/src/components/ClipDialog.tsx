...
export default function ClipDialog({ onClose }: { onClose: () => void }) {
  ...
  const clip = useMutation({
    mutationFn: () => api.nodes.clip(url.trim()),
    onSuccess: (node) => {
  ...
  const error = clip.error instanceof ApiError ? clip.error.message : null;
      ...
      >
        <div className="mb-4 flex items-center justify-between">
          <h2 className="flex items-center gap-2 text-base font-semibold text-ink">
            <Globe className="h-4 w-4 text-accent2" strokeWidth={1.75} /> {t("clip.title")}
          </h2>
        ...
        >
          ...
          />
          <p className="mt-2 text-xs text-dim">{t("clip.hint")}</p>
          {error && <p className="mt-2 text-sm text-red-400">{error}</p>}
          <div className="mt-4 flex justify-end">
            <Button type="submit" disabled={clip.isPending || !url.trim()}>
              {clip.isPending ? t("clip.working") : t("clip.save")}
            </Button>
          </div>
        </form>
      </div>
